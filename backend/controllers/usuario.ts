...
export const createUser = async(req:Request, resp:Response)=>{
  ...
  try{
    ...
    resp.status(200).json({
        msg:"El usuario ha sido creado exitosamente",usuarioc,
    });
    }catch(error){
...
try{
    ...
    res.status(200).json({
        msg:"El usuario se ha actualizado correctamente",actus,
    });
} catch(error){
    console.log(error);
    res.status(500).json({
...
res.status(404).json({
 ...
 res.status(200).json({
    msg:"La ciudad ha sido eliminada de forma correcta",usuario,
 });
};
